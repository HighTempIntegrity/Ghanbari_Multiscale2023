...
class ParallelTracks:
	...
	def set_localrange(self, first_id, last_id, track_id):
		...
		line_localid = '	local = laser_data.locals[lcl_id]\n'
		line_time = '	\'time_file\':\'3_time_locals_track%i\'\n'%(track_id+1)
		
		range_id = self.contents.index('for local in laser_data.locals:\n')
		time_id = self.contents.index('	\'time_file\':\'3_time_locals\',	# File name for saving all script duration values\n')
